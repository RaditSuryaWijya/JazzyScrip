-- JazzyGUI_v2.3.lua - Galaxy Edition (REFINED)
...
local colors = {
    primary = Color3.fromRGB(33, 150, 243),       -- Purple
...
makeToggle(catHideStats, "⚡ Enable Hide Stats", function(on)
    ...
    if on then
        HideStats.Enable()
        Notify.Send("Hide Stats ✨", "Hide Stats aktif! '-Jazzy-' berkilau di atas nama.", 3)
    else
        HideStats.Disable()
    ...
    if requestFunc then
        ...
        local testPayload = {
            embeds = {{
                ...
                description = "Your Discord webhook is working correctly!\n\nJazzyx GUI is ready to send fish notifications." .. filterText,
                color = 3447003,
                footer = {
                    text = "Jazzyx Webhook Test • " .. os.date("%m/%d/%Y %H:%M"),
